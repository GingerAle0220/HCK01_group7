...
void loop() {
  if (dataReceived) {
    ...
    if (input.length() == 8) {
      ...
      // パケット内の演奏順フィールド（index 2,3,4）は，MasterProcessing側が
      // INST_IDの番号（ピアノ=1,木琴=2,フルート=3）と同じ並びで送信するため，
      // index = INST_ID+1 で求まる。
      int orderFieldIndex = (INST_ID >= 1 && INST_ID <= 3) ? INST_ID + 1 : -1; // ドラム(4)等は未使用
