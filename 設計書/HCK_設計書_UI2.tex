...
図\ref{fig:1-4}に担当箇所である同期制御機能を中心に詳細化したFBS図を示す．また表\ref{tab:1-3}に，各機能の概要と実装上の要点をまとめる．本システムは，1台のPC（Processing）を指揮者UI，1台のMaster Arduinoを制御ハブ，4台のSlave Arduinoを各楽器ノードとして構成し，Processingから受け取った演奏設定をMasterが集約して各Slaveへ配信することで，複数ノードの演奏を協調制御する．
...
図\ref{fig:1-6}に，ProcessingからMasterへの送信処理における状態遷移を示す．Processing側では，待機中の状態においてBPM，演奏順，オクターブの各設定を受け付け，設定確定・送信操作が行われた時点でパケット生成処理へ遷移する．オクターブは$-1$〜$9$の範囲で入力し，送信時には$00$〜$10$の2桁固定長文字列に変換する．演奏順は楽器1〜3に対応するキー入力をもとに3桁の文字列として保持し，BPMは$30$〜$180$の範囲で3桁固定長の値として扱う．これらの値は\texttt{buildPacket()}によりエンコードされ，Masterへ送信可能な形式に整形される．
...
また，Processing側ではMasterからの応答を\texttt{serialEvent()}で受信し，状態に応じてUIロック制御を行う．Masterから\texttt{START}を受信した場合は演奏中のロック状態へ移行し，演奏順およびオクターブの変更を禁止する．\texttt{FINISH}を受信した場合はロックを解除し，再び待機中の編集可能状態へ戻る．この制御により，演奏中の誤操作を防ぎつつ，BPMのみを動的に変更できる入力インタフェースを実現している．
...
\begin{figure}[H]
    ...
    \caption{クマアニメーション（BearWindow）の表示画面}
    \label{fig:1-10}
\end{figure}
...
図\ref{fig:1-10}に，演奏中に表示されるクマアニメーションウィンドウの画面，図\ref{fig:1-11}にクマアニメーションの状態遷移図を示す．本ウィンドウは\texttt{BearWindow}クラスとして実装され，\texttt{PApplet.runSketch()}によりUI.pdeの\texttt{setup()}内で指揮者UIとは別プロセス・別ウィンドウとして起動される．毎フレーム，UI.pde側の\texttt{draw()}から\texttt{bearWin.bearBpm}および\texttt{bearWin.bearPlaying}へ現在のBPMと演奏中フラグが書き込まれ，BearWindow側はこれらの値のみを参照して描画とゲーム進行を行う一方向の同期構成となっている．
描画する画像（クマの歩行2種・ジャンプ・ダメージ，および空・地面・雲2種・木2種・石）はすべてドット絵のPNGとして用意し，UI.pde側の\texttt{setup()}内で\texttt{loadImage()}により読み込んだ後，\texttt{bearWin}の各フィールドへ代入してから\texttt{runSketch()}を呼び出している．これは，\texttt{runSketch()}で起動する別ウィンドウ内で直接\texttt{loadImage()}を行うと，セカンドアプレットの\texttt{sketchPath}がスケッチフォルダを正しく指さない場合があり，特にProcessing IDEの実行ボタンから起動した際に画像が読み込めず\texttt{null}になる既知の不具合を回避するための実装上の工夫である．
BearWindow内の処理は，\texttt{updateWorld()}による状態更新と，\texttt{draw()}による描画の2段に分かれる．\texttt{updateWorld()}では，現在のBPMから\texttt{getScrollSpeed()}を用いてスクロール速度を算出し，雲・木・地面・コイン・障害物の座標を毎フレーム更新する．スクロール速度はBPMに比例して増加するため，演奏のテンポが速いほどクマの歩行アニメーションおよび背景の流れが速くなり，テンポ感を視覚的に提示できる．地面（\texttt{ground.png}）は画像幅から1pxのオーバーラップ量を差し引いた間隔で水平方向にタイル状に並べ，\texttt{groundScrollX}を用いて連続的にループ表示することで，継ぎ目を視認させずに無限スクロールしているように見せている．
クマの状態は，\texttt{bearState}（歩行／ジャンプ／ダメージ）によって管理され，\texttt{updateBearState()}が毎フレーム呼び出されて現在の状態を判定する．ダメージ状態は障害物衝突直後の\texttt{hitFlash}が残っている間，ジャンプ状態は\texttt{bearJumpY}が0未満（空中にある）間とし，それ以外は歩行状態として\texttt{legAngle}の位相に応じて2種類の歩行画像を交互に表示する．ジャンプは\texttt{triggerJump()}がUI.pde側のスペースキー押下イベントから呼び出されることで発火し，\texttt{JUMP\_VELOCITY}を初期速度として\texttt{updateJump()}内で重力\texttt{GRAVITY}を用いた放物運動として処理する．
当たり判定は，描画に用いる画像のサイズとは独立したオブジェクトとして実装している．クマについては，画像の中心点（\texttt{bearHitCenterX()}，\texttt{bearHitCenterY()}）を中心とする半径\texttt{BEAR\_HIT\_RADIUS}の円を当たり判定として用い，コイン収集は円と点の距離判定，障害物との衝突は円と矩形の判定関数\texttt{circleRectOverlap()}によって行う．これにより，見た目の画像サイズを変更しても当たり判定の挙動に影響を与えずに済む設計としている．コインを収集すると獲得コイン数\texttt{currentRunScore}が加算され，障害物に接触すると一定数減算されるとともに，\texttt{collisionCooldown}による短時間の無敵時間を設けて連続減算を防いでいる．
演奏が開始されると，BearWindow側の\texttt{draw()}が\texttt{bearPlaying}の立ち上がり（前フレームまでfalseで今回trueになった瞬間）を検知して自動的に\texttt{startRun()}を呼び出し，ランの初期化と集計を開始する．演奏終了時には，UI.pde側の\texttt{completeCurrentRun()}からBearWindow側の\texttt{completeRun()}が呼び出され，当該ランのスコアをランキング配列\texttt{rankingScores}へ挿入する．UI.pde側は\texttt{hasResult()}でランの結果有無を確認し，結果がある場合は\texttt{drawRunResultPanel()}によって獲得コインとランキングを指揮者UI画面上に表示する．
...
\begin{figure}[H]
    ...
    \caption{UI，Masterへの送信制御およびクマアニメーション同期のフローチャート}
    \label{fig:1-9}
\end{figure}

\subsubsection{主要関数表}

表\ref{tab:1-6-1}，表\ref{tab:1-6-2}に示した主要関数について，制御の流れに沿ってその役割を述べる．
...
{\small
\begin{tabularx}{\textwidth}{|M{1.6cm}|M{3.0cm}|M{2.0cm}|Y|}
    \caption{主要関数表（UI.pde）}\label{tab:1-6-1}\\
    \hline
...
{\small
\begin{tabularx}{\textwidth}{|M{1.6cm}|M{2.8cm}|M{2.3cm}|Y|}
    \caption{主要関数表（BearWindow.pde）}\label{tab:1-6-2}\\
    \hline
    \textbf{戻り値} & \textbf{関数名} & \textbf{引数} & \textbf{役割} \\ \hline
    \endfirsthead
    \multicolumn{4}{c}{\tablename\ \thetable\ （続き）}\\
...
{\small
\begin{tabularx}{\textwidth}{|M{2.0cm}|M{3.0cm}|M{1.8cm}|Y|}
    \caption{主要変数および定数表（UI.pde）}\label{tab:1-7-1}\\
    \hline
...
{\small
\begin{tabularx}{\textwidth}{|M{2.0cm}|M{3.0cm}|M{1.8cm}|Y|}
    \caption{主要変数および定数表（BearWindow.pde）}\label{tab:1-7-2}\\
    \hline
    \textbf{型} & \textbf{変数名} & \textbf{初期値} & \textbf{役割} \\ \hline
    \endfirsthead
    \multicolumn{4}{c}{\tablename\ \thetable\ （続き）}\\
